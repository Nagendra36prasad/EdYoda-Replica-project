                        ...
                        if module_choice == 1:
                            ...
                            for i in range(len(modules)):
                                if modules[i]["module_ID"] == module_ID:
                                    module_found = True
                                    print("Enter new module details: ")
                                    module_name = input("Enter module Name: ")
                                    start_date = input("Enter start date in format(dd-mm-yyyy): ")
                                    end_date = input("Enter end date in format(dd-mm-yyyy): ")
                                    modules[i]["module_name"] = module_name
                                    modules[i]["start_date"] = start_date
                                    modules[i]["end_date"] = end_date
                                    update_flag = update_module("module.json", modules)
                                    print("Updated Module Successfully!") if update_flag else print("Failed to update the module.")
                                    break
                                if not module_found:
                                    print("No such Module ID present.")
